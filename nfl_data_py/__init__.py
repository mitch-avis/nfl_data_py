...
def import_pbp_data(
    ...
    columns = [col for col in columns if col not in ["season"]]

    if all([include_participation, len(columns) != 0]):
        columns = columns + [col for col in ["play_id", "old_game_id"] if col not in columns]
    ...
    if thread_requests and not cache:
        with ThreadPoolExecutor() as executor:
            ...
            futures_map = {
                executor.submit(
                    pd.read_parquet,
                    path=f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/pbp/play_by_play_{year}.parquet",
        ...
        for year in years:
            if cache:
                ...
                if not os.path.isdir(os.path.join(dpath, f"season={year}")):
                    raise ValueError(f"{year} cache file does not exist.")
                path = None
                # define path based on cache and alt_path variables
                for fname in filter(lambda x: f"season={year}" in x, os.listdir(dpath)):
            ...
            # load data
            try:
                data = pd.read_parquet(path, columns=columns if columns else None, engine="auto")
                ...
                if all([include_participation, year >= 2016, not cache]):
                    path = (
                        f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/pbp_participation/pbp_participation"
                        f"_{year}.parquet"
                    )
                    partic = pd.read_parquet(path)
    ...
    if pbp_data:
        plays = pd.concat(pbp_data).reset_index(drop=True)

    # converts float64 to float32, saves ~30% memory
    if downcast:
        print("Downcasting floats.")
        cols = plays.select_dtypes(include=[np.float64]).columns
        plays[cols] = plays[cols].astype(np.float32)
...
def cache_pbp(years, downcast=True, alt_path=None):
    ...
    for folder in [
        os.path.join(path, x) for x in os.listdir(path) for year in years if f"season={year}" in x
    ]:
        for file in os.listdir(folder):
            if file.endswith(".parquet"):
    ...
    for year in years:
        ...
        try:
            ...
            data = pd.read_parquet(
                f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/pbp/play_by_play_{year}.parquet",
                engine="auto",
            )
            ...
            if year >= 2016:
                path2 = (
                    f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/pbp_participation"
                    f"/pbp_participation_{year}.parquet"
                )
                part = pd.read_parquet(path2)
                raw = raw.merge(part, how="left", on=["play_id", "old_game_id"])

            if downcast:
                cols = raw.select_dtypes(include=[np.float64]).columns
                raw[cols] = raw[cols].astype(np.float32)
...
def see_pbp_cols():
    ...
    data = pd.read_parquet(
        f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/pbp/play_by_play_2020.parquet",
        engine="auto",
    )
    cols = data.columns
...
def see_weekly_cols():
    ...
    data = pd.read_parquet(
        f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/player_stats/player_stats_2020.parquet",
        engine="auto",
    )
    cols = data.columns
...
def __import_rosters(release, years, columns=None):
    ...
    if columns is not None and not (
        isinstance(columns, list) and all(isinstance(x, str) for x in columns) and len(columns) > 0
    ):
    ...
    rosters.rename(columns={"gsis_id": "player_id", "full_name": "player_name"}, inplace=True)
    if columns:
...
def import_weekly_rosters(years, columns=None):
    ...
    week_team_dates = pd.concat(
        [
            scheds[common_cols + ["home_team"]].rename(columns={"home_team": "team"}),
            scheds[common_cols + ["away_team"]].rename(columns={"away_team": "team"}),
        ]
    )
    roster_dates = pd.to_datetime(
        rosters.merge(week_team_dates, on=["season", "week", "team"], how="left").gameday
    )
...
def import_players():
    ...
    Returns:
        DataFrame
    """
    df = pd.read_parquet(f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/players/players.parquet")
    return df
...
def import_team_desc():
    ...
    # import desc data
    df = pd.read_csv(f"{NFLVERSE_RAW_URL}{NFLVERSE_PBP}/teams_colors_logos.csv")
...
def import_win_totals(years=None):
    ...
    logging.warning("The win totals data source is currently in flux and may be out of date.")
    ...
    df = pd.read_csv(url).loc[lambda df: df.game_id.notna()]
...
def import_officials(years=None):
    ...
    df = pd.read_csv(f"{NFLVERSE_RAW_URL}{NFLDATA}/officials.csv")
...
def import_sc_lines(years=None):
    ...
    # import data
    df = pd.read_csv(f"{NFLVERSE_RAW_URL}{NFLDATA}/sc_lines.csv")
...
def import_draft_picks(years=None):
    ...
    df = pd.read_parquet(
        f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/draft_picks/draft_picks.parquet",
        engine="auto",
    )
...
def import_draft_values(picks=None):
    ...
    # import data
    df = pd.read_csv(f"{NFLVERSE_RAW_URL}{NFLDATA}/draft_values.csv")
...
def import_combine_data(years=None, positions=None):
    ...
    # import data
    df = pd.read_parquet(
        f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/combine/combine.parquet",
        engine="auto",
    )
...
def import_contracts():
    ...
    df = pd.read_parquet(
        f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/contracts/historical_contracts.parquet"
    )
...
def import_ngs_data(stat_type, years=None):
    ...
    # import data
    url = f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/nextgen_stats/ngs_{stat_type}.parquet"
    data = pd.read_parquet(url)

    if len(years) > 0:
        data = data[data["season"].isin(list(years))]
...
def import_seasonal_pfr(s_type, years=None):
    ...
    url = f"{NFLVERSE_REPO_URL}{NFLVERSE_DATA}/pfr_advstats/advstats_season_{s_type}.parquet"
...
def clean_nfl_data(df):
    ...
    if "col_team" in df.columns:
        ...
        if "name" in df.columns:
            for player_name in player_col_tm_repl:
                df[df["name"] == player_name[0]] = df[df["name"] == player_name[0]].replace(
    ...
    if "team" in df.columns:
        ...
        if "name" in df.columns:
            for player_name in player_tm_repl:
                df[df["name"] == player_name[0]] = df[df["name"] == player_name[0]].replace(
                    {player_name[1]: player_name[2]}
                )
